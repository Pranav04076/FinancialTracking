...
def parse_pnb(file_path,user_id, db):
    ...
    for row in rows:
        ...
        #skip header row
        if row[0] and "DATE" in row[0].upper().replace(" ", ""):
            continue

        try:
            txn_date = parse_date(row[0])
            value_date  = parse_date(row[0])
        except Exception as e:
            print("DATE ERROR:", row[0], row[1], e)
            skipped+=1
            continue
        ...
        if row[4]:
            ...
            mode = get_mode(narration) or "Unknown"
